...
import com.hhlt.konsultado.controller.ConsultDataController;
...
public class FenPei {
    ...
    @Autowired
    private ChannelChildService childService;
...
//        List<String> keywordType = Arrays.asList("软件", "内容", "客服", "代运营", "直通车", "钻展");  // 特殊业务类型
            ...
            if (keywordChannel.contains(channel)) {
                ...
                map.put("planformId", getPlatId(informations, "SMB"));
                addOrUpdate(integer, map);
                result.setCode(1);
                result.setData("SMB");
            } else {
                logger.info("咨询渠道为：-------------> {}", channel);
                if ("直通车".equals(type) || "钻展".equals(type)) {
                    ...
                    result.setCode(1);
                    result.setData("济南软件");
                } else if ("代运营".equals(type) ) {
                    logger.info("业务类型为：-------------> {}", type);
                    result = daiYunYing(map, informations, keywordProvince, integer, fD.getOperate1());//代运营转化率
                } else {
                    logger.info("业务类型为：-------------> {}", type);
    ...
    public Result otherType(Map<String, Object> map, List<PlatformInformation> informations, Double num, int integer) {
        ...
        double avgRate = rateService.avgRate(trueDate) + num;     //按月份的平均转化率
        logger.info("本月的平均转化率：" + avgRate);

        List<PlatformRate> platList = rateService.platMap(trueDate);// 按月份的平台上月转化率  倒序
...
//            platCha.entrySet().forEach(next -> logger.info("平台名:" + next.getKey() + ",代运营分配的差值：" + next.getValue()));
    ...
    //各个部门已经分配的咨询数 从未分配的数量设为0
    private List<PlatformNum> getPlatformNum(String consultDate, List<PlatformRate> platList) {
        List<PlatformNum> platformNums = dataService.platformNums(consultDate);    //按月份 平台已经分配的数据量   包括其他部门的，需要去掉
        List<PlatformInformation> informations = informationService.listNew();
    ...
    //获取平台id
    private int getPlatId(List<PlatformInformation> informations, String platName) {
        for (PlatformInformation information : informations) {
            if (platName.equals(information.getPlatformName())) {
    ...
    //更新数据库
    private void updateDB(Map<String, Object> map) {
        ConsultData consultData = new ConsultData();
        transMap2Bean(map, consultData);
